...
PASS_IN_joined = BSTMT_LKP.join(
    CSTMT_LKP, BSTMT_LKP.sys_row_id == CSTMT_LKP.sys_row_id, 'inner')
CSTMT_LKP.join(ESTMT_LKP, CSTMT_LKP.sys_row_id ==
               ESTMT_LKP.sys_row_id, 'inner')
ESTMT_LKP.join(LkpECF_STATUSCD, ESTMT_LKP.sys_row_id ==
               LkpECF_STATUSCD.sys_row_id, 'inner')
LkpECF_STATUSCD.join(LkpTIMSTNaturalHash, LkpECF_STATUSCD.sys_row_id ==
                     LkpTIMSTNaturalHash.sys_row_id, 'inner')
LkpTIMSTNaturalHash.join(
    NSTMT_LKP, LkpTIMSTNaturalHash.sys_row_id == NSTMT_LKP.sys_row_id, 'inner')
NSTMT_LKP.join(NULLSTMT_LKP, NSTMT_LKP.sys_row_id ==
               NULLSTMT_LKP.sys_row_id, 'inner')
NULLSTMT_LKP.join(PSTMT_LKP, NULLSTMT_LKP.sys_row_id ==
                  PSTMT_LKP.sys_row_id, 'inner')
...
TSTMT_LKP.join(Timst_INTRATE_Lkup, TSTMT_LKP.sys_row_id ==
               Timst_INTRATE_Lkup.sys_row_id, 'inner')
...
PASS_IN = PASS_IN.withColumn("CDLastIntReprcDt", when((Not(Timst_INTRATE_Lkup.NOTFOUND), when((trim(SEQ_IN.TIMST_INTRATE) == Timst_INTRATE_Lkup.TIMST_INTRATE)).otherwise((Timst_INTRATE_Lkup.LAST_INT_REPRC_DT))).otherwise((DATA_DATE))).otherwise(trim(SEQ_IN.TIMST_RENLAST))).select(
    ...
    (when((length(SEQ_IN.TIMST_BALCUR) == lit(0)), (lit(0))).otherwise(
        SEQ_IN.TIMST_BALCUR)).alias('BANK_SHR_BAL'),
    (when((length(SEQ_IN.TIMST_INTRATE) == lit(0)), (lit(0))).otherwise(
    ...
    (when((length(SEQ_IN.TIMST_INTENP) == lit(0)), (lit(0))).otherwise(
        SEQ_IN.TIMST_INTENP)).alias('MTD_INT_ACCR_AMT'),
...
ACCT_BAL_OUT_joined = ACCT_Lkp.join(
    ODH_LKP, ACCT_Lkp.sys_row_id == ODH_LKP.sys_row_id, 'inner')
...
ACCT_BAL_OUT = ACCT_BAL_OUT.withColumn("RptRecInd", SetRptRecInd(ACCT_Lkp.OPEN_CLOSE_CD, ACCT_Lkp.CLOSE_DT, PASS_IN.CUR_BAL, PASS_IN.BANK_SHR_BAL, .DATA_DATE)).select(
    ...
    (when((trim(ACCT_Lkp.INT_RATE_TYP) == lit('V')), ((PASS_IN.TIMST_INTRATE -
     PASS_IN.TIMST_INTINDEX) * lit(100))).otherwise(lit(0))).alias('INT_INDX_BASE_RATE'),
    ...
    (when((ACCT_Lkp.OPEN_CLOSE_CD == lit('C')), (lit(0))
          ).otherwise(PASS_IN.ESTMT_CNT)).alias('ESTMT_CNT'),
    (when((ACCT_Lkp.OPEN_CLOSE_CD == lit('C')), (lit(0))).otherwise(
...
ACCT_OUT = ACCT_OUT.withColumn("RptRecInd", SetRptRecInd(ACCT_Lkp.OPEN_CLOSE_CD, ACCT_Lkp.CLOSE_DT, PASS_IN.CUR_BAL, PASS_IN.BANK_SHR_BAL, .DATA_DATE)).select(
    ...
    ACCT_Lkp.SCNDY_LVL_INT_MRGN_ACTION_CD.alias(
        'SCNDY_LVL_INT_MRGN_ACTION_CD'),
    ...
    (when((RptRecInd == lit('N')), (RptRecInd)).otherwise(
        ACCT_Lkp.RPT_REC_IND)).alias('RPT_REC_IND'),
...
Timst_INTRATE_OUT_joined = ACCT_Lkp.join(
    ODH_LKP, ACCT_Lkp.sys_row_id == ODH_LKP.sys_row_id, 'inner')
ODH_LKP.join(PASS_IN, ODH_LKP.sys_row_id == PASS_IN.sys_row_id, 'inner')
...
SEQ_IN_CONV2_joined = ABCDDH_LKP.join(
    ACCT_BAL_OUT, ABCDDH_LKP.sys_row_id == ACCT_BAL_OUT.sys_row_id, 'inner')
