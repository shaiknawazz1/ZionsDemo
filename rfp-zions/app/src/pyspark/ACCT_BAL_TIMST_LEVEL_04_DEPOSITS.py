...
PASS_IN = PASS_IN.withColumn("CDLastIntReprcDt", when((Not(Timst_INTRATE_Lkup.NOTFOUND) , when((trim(SEQ_IN.TIMST_INTRATE) == Timst_INTRATE_Lkup.TIMST_INTRATE )).otherwise((Timst_INTRATE_Lkup.LAST_INT_REPRC_DT ))).otherwise((DATA_DATE) )).otherwise(trim(SEQ_IN.TIMST_RENLAST))).select(
	...
	(when(((SEQ_IN.TIMST_BALCUR != lit(0)) &(SEQ_IN.TIMST_INTRATE != lit(0)) , when((SEQ_IN.TIMST_INTCODE == lit('B') ).otherwise(SEQ_IN.TIMST_BALCUR *(SEQ_IN.TIMST_INTRATE) /(udf_days_in_year(lit(DATA_DATE))) ).otherwise(when((SEQ_IN.TIMST_INTCODE == lit('C') )).otherwise(((SEQ_IN.TIMST_BALCAGR / substring((last_day(to_date(lit(DATA_DATE), 'yyyy-MM-dd'))) ))).otherwise((lit(2))) *(SEQ_IN.TIMST_INTRATE) /(udf_days_in_year(lit(DATA_DATE))) ))).otherwise((lit(0)) )).otherwise(lit(0)))).alias('INT_PERDIEM_AMT'),
	...
	(when((LkpECF_STATUSCD.OUT_1 == lit('C') , None ).otherwise(when((udf_convert_encoding(trim(SEQ_IN.TIMST_RATENEXT)).otherwise(lit('D')) > udf_convert_encoding(trim(SEQ_IN.TIMST_ISSDATE))).otherwise((lit('D')) ))).otherwise((trim(SEQ_IN.TIMST_RATENEXT) )).otherwise(trim(SEQ_IN.TIMST_RENNEXT)))).alias('NEXT_INT_REPRC_DT'),
	...
	(when((udf_convert_encoding(trim(SEQ_IN.TIMST_INTDTCHG), lit('D')) > udf_convert_encoding(trim(SEQ_IN.TIMST_ISSDATE)).otherwise(lit('D')) )).otherwise((trim(SEQ_IN.TIMST_INTDTCHG) )).otherwise(None)).alias('LAST_INT_REPRC_DT'),
	...
	(when(((SEQ_IN.TIMST_BALCUR != lit(0)) &(SEQ_IN.TIMST_INTRATE != lit(0)) , when((SEQ_IN.TIMST_INTCODE == lit('B') ).otherwise(SEQ_IN.TIMST_BALCUR *(SEQ_IN.TIMST_INTRATE) /(udf_days_in_year(lit(DATA_DATE))) ).otherwise(when((SEQ_IN.TIMST_INTCODE == lit('C') )).otherwise(((SEQ_IN.TIMST_BALCAGR / substring((last_day(to_date(lit(DATA_DATE), 'yyyy-MM-dd'))) ))).otherwise((lit(2))) *(SEQ_IN.TIMST_INTRATE) /(udf_days_in_year(lit(DATA_DATE))) ))).otherwise((lit(0)) )).otherwise(lit(0)))).alias('BANK_SHR_INT_PERDIEM_AMT'),
...
DepositTIMSTHash.write.format('csv').option('header','false').mode('overwrite').option('sep',',').csv('DepositTIMSTHash')
...
AcctBal_Timst_INTRATE_Dpst_Hash.write.format('csv').option('header','false').mode('overwrite').option('sep',',').csv('AcctBal_Timst_INTRATE_Dpst_Hash')
