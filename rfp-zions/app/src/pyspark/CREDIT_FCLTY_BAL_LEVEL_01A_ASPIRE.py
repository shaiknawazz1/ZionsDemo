#Code converted on 2023-04-26 15:37:31
import os
...
dbutils.widgets.text(name = 'FMT_DIR', defaultValue = '/nas_pp/test/dw/fmt')
FMT_DIR = dbutils.widgets.get("FMT_DIR")

dbutils.widgets.text(name = 'ECF_DIR', defaultValue = '/nas_pp/test/dw/data/ecf')
ECF_DIR = dbutils.widgets.get("ECF_DIR")

dbutils.widgets.text(name = 'TEMP_DIR', defaultValue = '/nas_pp/test/dw/temp')
...
dbutils.widgets.text(name = 'REC_DIR', defaultValue = '/nas_pp/test/dw/recovery')
REC_DIR = dbutils.widgets.get("REC_DIR")

dbutils.widgets.text(name = 'SKEY_DIR', defaultValue = '/nas_pp/test/dw/stg_keys')
SKEY_DIR = dbutils.widgets.get("SKEY_DIR")

dbutils.widgets.text(name = 'APPL_CD', defaultValue = 'LS')
...
dbutils.widgets.text(name = 'DATA_DATE', defaultValue = '20220831')
...
CONTRACT_GENERAL_LEDGER_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_adhocreport_contract_general_ledger.ds", sep=',', header='false')
...
aspire_adhocreport_contract_payment_summary_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_adhocreport_contract_payment_summary.ds", sep=',', header='false')
...
spire_adhocreport_contract_equipment_summary_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_adhocreport_contract_equipment_summary.ds", sep=',', header='false')
...
CONTRACT_MASTER_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_acct_driver_file.ds", sep=',', header='false')
...
aspire_dbo_doc_gen_con_idc_zcc_idc_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_doc_gen_con_idc_zcc_idc.ds", sep=',', header='false')
...
aspire_dbo_lti_rpt_renewal_status_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_lti_rpt_renewal_status.ds", sep=',', header='false')
...
earnings_schedule_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_earnings_schedule.ds", sep=',', header='false')
...
bookvalue_booked_contract_component_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_bookvalue_booked_contract_component.ds", sep=',', header='false')
...
aspire_dbo_doc_gen_con_contract_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_doc_gen_con_contract.ds", sep=',', header='false')
...
LKP_NATL_ACCT = spark.read.csv(""+DS_DIR+"/aspire/natl_key/aspire_acct_natl_key.ds", sep=',', header='false')
...
aspire_dbo_doc_gen_con_contract_udf_extensions_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_doc_gen_con_contract_udf_extensions.ds", sep=',', header='false')
...
CONTRACT_AGING_DS = spark.read.csv(""+DS_DIR+"/aspire/aspire_adhocreport_contract_aging.ds", sep=',', header='false')
...
ASPIRE_DBO_LTI_VALUES_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_lti_values.ds", sep=',', header='false')
...
CONTRACT_MASTER = CONTRACT_MASTER_IN.select(
	...
	(lit('{APPL_CD}') + lit('|') + CONTRACT_MASTER_IN.product_code + lit('|') + substring(CONTRACT_MASTER_IN.contract_type_code , lit(1) , lit(3))).alias('PROD_DTL_NATL_KEY'),
	(lit('{APPL_CD}')).alias('APPL_CD_LX'),
	(when((trim(CONTRACT_MASTER_IN.is_terminated) == lit('True') ),(lit('C') )).otherwise(lit('O'))).alias('OPEN_CLOSE_CD'),
	(lit('LX')).alias('APPLICATIONS'),
	(lit('103')).alias('IN_2'),
	CONTRACT_MASTER_IN.months_renewal_maturity_date.alias('months_renewal_maturity_date'),
	CONTRACT_MASTER_IN.acceptance_date.alias('acceptance_date'),
	CONTRACT_MASTER_IN.account_dist_code.alias('account_dist_code'),
	CONTRACT_MASTER_IN.account_dist_code_description.alias('account_dist_code_description'),
	...
	CONTRACT_MASTER_IN.bill_to_invoice_group_description.alias('bill_to_invoice_group_description'),
	CONTRACT_MASTER_IN.bill_to_is_invoice_group.alias('bill_to_is_invoice_group'),
	...
	CONTRACT_MASTER_IN.chargeoff_effective_date.alias('chargeoff_effective_date'),
	...
	CONTRACT_MASTER_IN.contract_type_description.alias('contract_type_description'),
	CONTRACT_MASTER_IN.customer_alt_name.alias('customer_alt_name'),
	CONTRACT_MASTER_IN.customer_business_individual_indicator.alias('customer_business_individual_indicator'),
	...
	CONTRACT_MASTER_IN.delinquency_code_description.alias('delinquency_code_description'),
	...
	CONTRACT_MASTER_IN.invoice_code_description.alias('invoice_code_description'),
	...
	CONTRACT_MASTER_IN.nonaccrual_effective_date.alias('nonaccrual_effective_date'),
	CONTRACT_MASTER_IN.payment_adjustment_method.alias('payment_adjustment_method'),
	...
	CONTRACT_MASTER_IN.rent_transaction_code_code.alias('rent_transaction_code_code'),
	CONTRACT_MASTER_IN.rent_transaction_code_description.alias('rent_transaction_code_description'),
	CONTRACT_MASTER_IN.rent_transaction_code_invoice_description.alias('rent_transaction_code_invoice_description'),
	CONTRACT_MASTER_IN.rent_transaction_code_oid.alias('rent_transaction_code_oid'),
	...
	CONTRACT_MASTER_IN.sales_rep_email_address.alias('sales_rep_email_address'),
	...
	CONTRACT_MASTER_IN.customer_oid_from_customer.alias('customer_oid_from_customer'),
	CONTRACT_MASTER_IN.value_zr.alias('value_zr'),
	CONTRACT_MASTER_IN.oid_from_chrg_dwn_amt.alias('oid_from_chrg_dwn_amt'),
	CONTRACT_MASTER_IN.contract_oid_from_eff_rate_chg.alias('contract_oid_from_eff_rate_chg'),
	...
	CONTRACT_MASTER_IN.contract_oid_from_contract_aging.alias('contract_oid_from_contract_aging'),
	...
	CONTRACT_MASTER_IN.contract_oid_from_contract_general_ledger.alias('contract_oid_from_contract_general_ledger'),
	CONTRACT_MASTER_IN.original_lease_receivable_balance.alias('original_lease_receivable_balance'),
	CONTRACT_MASTER_IN.original_residual_receivable.alias('original_residual_receivable'),
	CONTRACT_MASTER_IN.contract_oid_from_contract_equipment_summary.alias('contract_oid_from_contract_equipment_summary'),
	CONTRACT_MASTER_IN.equipment_cost.alias('equipment_cost'),
	CONTRACT_MASTER_IN.contract_oid_from_rpt_renewal_status.alias('contract_oid_from_rpt_renewal_status'),
	...
	CONTRACT_MASTER_IN.customer_ssn_tax_id_number.alias('customer_ssn_tax_id_number'),
	...
	CONTRACT_MASTER_IN.udfcontract_year_of_libor_used.alias('udfcontract_year_of_libor_used'),
...
ASPIRE_MISC_CUR_AND_BANK_SHR_BAL_IN = spark.read.csv(""+DS_DIR+"/aspire/acct/aspire_cur_and_bank_shr_bal.ds", sep=',', header='false')
...
PMT_SCHED_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_doc_gen_con_payment_schedule.ds", sep=',', header='false')
...
aspire_dbo_contract_values_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_contract_values.ds", sep=',', header='false')
...
ASPIRE_GEN_CON_PRIN_AND_INT_TOTALS_IN = spark.read.csv(""+DS_DIR+"/aspire/aspire_dbo_doc_gen_con_prin_and_int_totals.ds", sep=',', header='false')
...
earnings_schedule_IN_CONV2 = ODATE_OUT.join(ODATE, [ODATE.DATA_DATE == ODATE_OUT.'__JOBVAR_ENCLOSURE_OPEN__DATA_DATE__JOBVAR_ENCLOSURE_CLOSE__'], 'LEFT_OUTER').select(
...
LKP_MASTER_OUT1 = CONTRACT_MASTER.join(LKP_NATL_ACCT, [LKP_NATL_ACCT.NATL_KEY == CONTRACT_MASTER.MST_ACCT_NATL_KEY], 'LEFT_OUTER').join(CONTRACT_GENERAL_LEDGER_IN, [CONTRACT_GENERAL_LEDGER_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(earnings_schedule_OUT, [earnings_schedule_OUT.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(bookvalue_booked_contract_component_IN, [bookvalue_booked_contract_component_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(aspire_dbo_doc_gen_con_idc_zcc_idc_IN, [aspire_dbo_doc_gen_con_idc_zcc_idc_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(aspire_dbo_lti_rpt_renewal_status_IN, [aspire_dbo_lti_rpt_renewal_status_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(aspire_dbo_doc_gen_con_contract_IN, [aspire_dbo_doc_gen_con_contract_IN.contract_id == CONTRACT_MASTER.contract_id], 'LEFT_OUTER').join(aspire_adhocreport_contract_payment_summary_IN, [aspire_adhocreport_contract_payment_summary_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(ASPIRE_DBO_LTI_VALUES_IN, [ASPIRE_DBO_LTI_VALUES_IN.oid == CONTRACT_MASTER.oid], 'LEFT_OUTER').join(ASPIRE_GEN_CON_PRIN_AND_INT_TOTALS_IN, [ASPIRE_GEN_CON_PRIN_AND_INT_TOTALS_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(spire_adhocreport_contract_equipment_summary_IN, [spire_adhocreport_contract_equipment_summary_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(aspire_dbo_doc_gen_con_contract_udf_extensions_IN, [aspire_dbo_doc_gen_con_contract_udf_extensions_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(aspire_dbo_contract_values_IN, [aspire_dbo_contract_values_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(ASPIRE_MISC_CUR_AND_BANK_SHR_BAL_IN, [ASPIRE_MISC_CUR_AND_BANK_SHR_BAL_IN.MST_ACCT_NATL_KEY == CONTRACT_MASTER.MST_ACCT_NATL_KEY], 'LEFT_OUTER').join(PMT_SCHED_IN, [PMT_SCHED_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(Earnings_SUM_IN, [Earnings_SUM_IN.contract_oid == CONTRACT_MASTER.contract_oid], 'LEFT_OUTER').join(ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN, [ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.contract_id == CONTRACT_MASTER.contract_id], 'LEFT_OUTER').select(
	...
	CONTRACT_MASTER.months_renewal_maturity_date.alias('months_renewal_maturity_date'),
	CONTRACT_MASTER.acceptance_date.alias('acceptance_date'),
	CONTRACT_MASTER.account_dist_code.alias('account_dist_code'),
	CONTRACT_MASTER.account_dist_code_description.alias('account_dist_code_description'),
	...
	CONTRACT_MASTER.bill_to_invoice_group_description.alias('bill_to_invoice_group_description'),
	...
	CONTRACT_MASTER.contract_type_description.alias('contract_type_description'),
	...
	CONTRACT_MASTER.delinquency_code_description.alias('delinquency_code_description'),
	...
	CONTRACT_MASTER.nonaccrual_effective_date.alias('nonaccrual_effective_date'),
	CONTRACT_MASTER.payment_adjustment_method.alias('payment_adjustment_method'),
	...
	CONTRACT_MASTER.rent_transaction_code_code.alias('rent_transaction_code_code'),
	CONTRACT_MASTER.rent_transaction_code_description.alias('rent_transaction_code_description'),
	CONTRACT_MASTER.rent_transaction_code_invoice_description.alias('rent_transaction_code_invoice_description'),
	CONTRACT_MASTER.rent_transaction_code_oid.alias('rent_transaction_code_oid'),
	...
	CONTRACT_MASTER.customer_oid_from_customer.alias('customer_oid_from_customer'),
	CONTRACT_MASTER.value_zr.alias('value_zr'),
	CONTRACT_MASTER.oid_from_chrg_dwn_amt.alias('oid_from_chrg_dwn_amt'),
	CONTRACT_MASTER.contract_oid_from_eff_rate_chg.alias('contract_oid_from_eff_rate_chg'),
	...
	CONTRACT_MASTER.contract_oid_from_contract_aging.alias('contract_oid_from_contract_aging'),
	...
	CONTRACT_MASTER.contract_oid_from_contract_general_ledger.alias('contract_oid_from_contract_general_ledger'),
	CONTRACT_MASTER.original_lease_receivable_balance.alias('original_lease_receivable_balance'),
	CONTRACT_MASTER.original_residual_receivable.alias('original_residual_receivable'),
	CONTRACT_MASTER.contract_oid_from_contract_equipment_summary.alias('contract_oid_from_contract_equipment_summary'),
	CONTRACT_MASTER.equipment_cost.alias('equipment_cost'),
	CONTRACT_MASTER.contract_oid_from_rpt_renewal_status.alias('contract_oid_from_rpt_renewal_status'),
	...
	CONTRACT_MASTER.customer_ssn_tax_id_number.alias('customer_ssn_tax_id_number'),
	CONTRACT_MASTER.customer_business_individual_indicator.alias('customer_business_individual_indicator'),
	...
	CONTRACT_MASTER.udfcontract_year_of_libor_used.alias('udfcontract_year_of_libor_used'),
	CONTRACT_GENERAL_LEDGER_IN.last_month_end_date.alias('last_month_end_date'),
	CONTRACT_GENERAL_LEDGER_IN.deferred_income_lease.alias('deferred_income_lease'),
	CONTRACT_GENERAL_LEDGER_IN.deferred_income_residual.alias('deferred_income_residual'),
	earnings_schedule_OUT.earnings_type_oid.alias('earnings_type_oid'),
	earnings_schedule_OUT.earnings.alias('earnings'),
	bookvalue_booked_contract_component_IN.accumulated_idc.alias('accumulated_idc'),
	bookvalue_booked_contract_component_IN.accumulated_fee.alias('accumulated_fee'),
	...
	aspire_dbo_lti_rpt_renewal_status_IN.renewal_maturity_date.alias('renewal_maturity_date_lti_rpt'),
	aspire_dbo_doc_gen_con_contract_IN.last_payment_amount.alias('last_payment_amount'),
	aspire_adhocreport_contract_payment_summary_IN.last_rental_paid_date.alias('last_rental_paid_date'),
	ASPIRE_DBO_LTI_VALUES_IN.datavalue.alias('datavalue'),
	ASPIRE_GEN_CON_PRIN_AND_INT_TOTALS_IN.interest_ltd.alias('interest_ltd'),
	spire_adhocreport_contract_equipment_summary_IN.equipment_cost.alias('equipment_cost_equipment_summary'),
	aspire_dbo_doc_gen_con_contract_udf_extensions_IN.value_zr.alias('value_zr_udf_extn_euipment'),
	...
	CONTRACT_GENERAL_LEDGER_IN.ytd_accrued_interest.alias('ytd_accrued_interest'),
	CONTRACT_GENERAL_LEDGER_IN.ytd_non_accrual_interest_income.alias('ytd_non_accrual_interest_income'),
	aspire_adhocreport_contract_payment_summary_IN.current_payment_amount.alias('current_payment_amount'),
	CONTRACT_GENERAL_LEDGER_IN.mtd_earned_income_lease.alias('mtd_earned_income_lease'),
	CONTRACT_GENERAL_LEDGER_IN.mtd_earned_income_residual.alias('mtd_earned_income_residual'),
	CONTRACT_GENERAL_LEDGER_IN.mtd_interest_income.alias('mtd_interest_income'),
	...
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_1.alias('rent_due_period_1'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_10.alias('rent_due_period_10'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_2.alias('rent_due_period_2'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_3.alias('rent_due_period_3'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_4.alias('rent_due_period_4'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_5.alias('rent_due_period_5'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_6.alias('rent_due_period_6'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_7.alias('rent_due_period_7'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_8.alias('rent_due_period_8'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.rent_due_period_9.alias('rent_due_period_9'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.oldest_rent_due_date.alias('oldest_rent_due_date_contract_aging'),
	CONTRACT_GENERAL_LEDGER_IN.principal_balance.alias('principal_balance'),
	ASPIRE_MISC_CUR_AND_BANK_SHR_BAL_IN.'__JOBVAR_ENCLOSURE_OPEN__DATA_DATE__JOBVAR_ENCLOSURE_CLOSE__'.alias('DATA_DATE'),
	PMT_SCHED_IN.amount.alias('amount_pmt_sched'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_2.alias('late_fee_due_period_2'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_3.alias('late_fee_due_period_3'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_4.alias('late_fee_due_period_4'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_5.alias('late_fee_due_period_5'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_6.alias('late_fee_due_period_6'),
	ASPIRE_ADHOCREPORT_CONTRACT_AGING_IN.late_fee_due_period_7.alias('late_fee_due_period_7'),
	Earnings_SUM_IN.SUM_EARNINGS.alias('SUM_EARNINGS'),
	aspire_adhocreport_contract_payment_summary_IN.current_contract_receivable.alias('current_contract_receivable'),
	CONTRACT_GENERAL_LEDGER_IN.ytd_interest_income.alias('ytd_interest_income'),
	CONTRACT_GENERAL_LEDGER_IN.ytd_earned_income_lease.alias('ytd_earned_income_lease'),
	CONTRACT_GENERAL_LEDGER_IN.ytd_earned_income_residual.alias('ytd_earned_income_residual'),
	CONTRACT_GENERAL_LEDGER_IN.ltd_earned_income_lease.alias('ltd_earned_income_lease'),
	CONTRACT_GENERAL_LEDGER_IN.ltd_earned_income_residual.alias('ltd_earned_income_residual'),
	CONTRACT_GENERAL_LEDGER_IN.ltd_interest_income.alias('ltd_interest_income'),
	...
	aspire_dbo_doc_gen_con_contract_IN.extended_maturity_date.alias('extended_maturity_date')
)
...
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("Matdtnew1", when((LKP_MASTER_OUT1.extended_maturity_date == None ),(LKP_MASTER_OUT1.maturity_date )).otherwise(LKP_MASTER_OUT1.extended_maturity_date))
...
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("RenwMatDt", RenMatDt .cast(date))
...
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("OldRentDueDt", when((LKP_MASTER_OUT1.oldest_rent_due_date_contract_aging == None ),(lit('0001-01-01') )).otherwise(LKP_MASTER_OUT1.oldest_rent_due_date_contract_aging .cast(date)))
...
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("Svrentdueperiod", trim(LKP_MASTER_OUT1.rent_due_period_2) + trim(LKP_MASTER_OUT1.rent_due_period_3) + trim(LKP_MASTER_OUT1.rent_due_period_4) + trim(LKP_MASTER_OUT1.rent_due_period_5) + trim(LKP_MASTER_OUT1.rent_due_period_6) + trim(LKP_MASTER_OUT1.rent_due_period_7))
	
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("TotPdueFeeAmt", trim(LKP_MASTER_OUT1.late_fee_due_period_2) + trim(LKP_MASTER_OUT1.late_fee_due_period_3) + trim(LKP_MASTER_OUT1.late_fee_due_period_4) + trim(LKP_MASTER_OUT1.late_fee_due_period_5) + trim(LKP_MASTER_OUT1.late_fee_due_period_6) + trim(LKP_MASTER_OUT1.late_fee_due_period_7))
	
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("MatDt", when((LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt == None ),(LKP_MASTER_OUT1.maturity_date )).otherwise(RenwMatDt))
...
LKP_MASTER_OUT21 = LKP_MASTER_OUT21.withColumn("StageVar", (when(((LKP_MASTER_OUT1.rent_due_period_2) IS NOT None , (LKP_MASTER_OUT1.rent_due_period_2) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_3) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_3) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_4) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_4) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_5) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_5) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_6) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_6) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_7) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_7) )).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_8) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_8) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_9) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_9) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_10) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_10) )).otherwise(lit(0)))))))))))).select(
	...
	LKP_MASTER_OUT1.ltd_earned_income_residual.alias('ltd_earned_income_residual'),
	LKP_MASTER_OUT1.ytd_earned_income_lease.alias('ytd_earned_income_lease'),
	LKP_MASTER_OUT1.ytd_earned_income_residual.alias('ytd_earned_income_residual'),
	LKP_MASTER_OUT1.mtd_earned_income_lease.alias('mtd_earned_income_lease'),
	LKP_MASTER_OUT1.mtd_earned_income_residual.alias('mtd_earned_income_residual'),
	...
	LKP_MASTER_OUT1.ltd_earned_income_residual.alias('ltd_earned_income_residual_1'),
	...
	LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt.alias('renewal_maturity_date_lti_rpt'),
	...
	(when((LKP_MASTER_OUT1.OPEN_CLOSE_CD == lit('C') , lit(0) ).otherwise(when((LKP_MASTER_OUT1.BANK_SHR_BAL < lit(2) ).otherwise(lit(0) ).otherwise(when((Matdtnew1 <= AsOfDt )).otherwise((LKP_MASTER_OUT1.principal_balance ))).otherwise((IFF(PiDueAmt > lit(15) ))).otherwise((PiDueAmt )).otherwise(lit(0)))))).alias('pdue_pi_amt_new'),
	LKP_MASTER_OUT1.principal_balance.alias('principal_balance'),
	(when((LKP_MASTER_OUT1.OPEN_CLOSE_CD == lit('C') , lit(0) ).otherwise(when((LKP_MASTER_OUT1.OPEN_CLOSE_CD == lit('C') ).otherwise(lit(0) ).otherwise(when((LKP_MASTER_OUT1.BANK_SHR_BAL < lit(2) ).otherwise(lit(0) ).otherwise(IFF(MatDt <= LKP_MASTER_OUT1.'{DATA_DATE}' )).otherwise((LKP_MASTER_OUT1.principal_balance ))).otherwise((IFF(PiDueAmt > lit(15) ))).otherwise((PiDueAmt )).otherwise(lit(0))))))).alias('pdue_pi_amt'),
...
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("Matdtnew1", when((LKP_MASTER_OUT1.extended_maturity_date == None ),(LKP_MASTER_OUT1.maturity_date )).otherwise(LKP_MASTER_OUT1.extended_maturity_date))
...
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("OldRentDueDt", when((LKP_MASTER_OUT1.oldest_rent_due_date_contract_aging == None ),(lit('0001-01-01') )).otherwise(LKP_MASTER_OUT1.oldest_rent_due_date_contract_aging .cast(date)))
...
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("Svrentdueperiod", trim(LKP_MASTER_OUT1.rent_due_period_2) + trim(LKP_MASTER_OUT1.rent_due_period_3) + trim(LKP_MASTER_OUT1.rent_due_period_4) + trim(LKP_MASTER_OUT1.rent_due_period_5) + trim(LKP_MASTER_OUT1.rent_due_period_6) + trim(LKP_MASTER_OUT1.rent_due_period_7))
	
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("TotPdueFeeAmt", trim(LKP_MASTER_OUT1.late_fee_due_period_2) + trim(LKP_MASTER_OUT1.late_fee_due_period_3) + trim(LKP_MASTER_OUT1.late_fee_due_period_4) + trim(LKP_MASTER_OUT1.late_fee_due_period_5) + trim(LKP_MASTER_OUT1.late_fee_due_period_6) + trim(LKP_MASTER_OUT1.late_fee_due_period_7))
	
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("MatDt", when((LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt == None ),(LKP_MASTER_OUT1.maturity_date )).otherwise(RenwMatDt))
...
LKP_MASTER_OUT2 = LKP_MASTER_OUT2.withColumn("StageVar", (when(((LKP_MASTER_OUT1.rent_due_period_2) IS NOT None , (LKP_MASTER_OUT1.rent_due_period_2) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_3) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_3) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_4) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_4) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_5) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_5) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_6) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_6) ).otherwise(lit(0)) +(when(((LKP_MASTER_OUT1.rent_due_period_7) IS NOT None ).otherwise((LKP_MASTER_OUT1.rent_due_period_7) )).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_8) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_8) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_9) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_9) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.rent_due_period_10) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.rent_due_period_10) )).otherwise(lit(0)))))))))))).select(
	...
	LKP_MASTER_OUT1.months_renewal_maturity_date.alias('months_renewal_maturity_date'),
	LKP_MASTER_OUT1.acceptance_date.alias('acceptance_date'),
	LKP_MASTER_OUT1.account_dist_code.alias('account_dist_code'),
	LKP_MASTER_OUT1.account_dist_code_description.alias('account_dist_code_description'),
	...
	LKP_MASTER_OUT1.bill_to_invoice_group_description.alias('bill_to_invoice_group_description'),
	...
	LKP_MASTER_OUT1.contract_type_description.alias('contract_type_description'),
	...
	LKP_MASTER_OUT1.delinquency_code_description.alias('delinquency_code_description'),
	...
	LKP_MASTER_OUT1.maturity_date.alias('maturity_date'),
	LKP_MASTER_OUT1.nonaccrual_effective_date.alias('nonaccrual_effective_date'),
	LKP_MASTER_OUT1.payment_adjustment_method.alias('payment_adjustment_method'),
	...
	LKP_MASTER_OUT1.rent_transaction_code_code.alias('rent_transaction_code_code'),
	LKP_MASTER_OUT1.rent_transaction_code_description.alias('rent_transaction_code_description'),
	LKP_MASTER_OUT1.rent_transaction_code_invoice_description.alias('rent_transaction_code_invoice_description'),
	LKP_MASTER_OUT1.rent_transaction_code_oid.alias('rent_transaction_code_oid'),
	...
	LKP_MASTER_OUT1.customer_oid_from_customer.alias('customer_oid_from_customer'),
	LKP_MASTER_OUT1.value_zr.alias('value_zr'),
	LKP_MASTER_OUT1.oid_from_chrg_dwn_amt.alias('oid_from_chrg_dwn_amt'),
	LKP_MASTER_OUT1.contract_oid_from_eff_rate_chg.alias('contract_oid_from_eff_rate_chg'),
	...
	LKP_MASTER_OUT1.contract_oid_from_contract_aging.alias('contract_oid_from_contract_aging'),
	...
	LKP_MASTER_OUT1.contract_oid_from_contract_general_ledger.alias('contract_oid_from_contract_general_ledger'),
	LKP_MASTER_OUT1.original_lease_receivable_balance.alias('original_lease_receivable_balance'),
	LKP_MASTER_OUT1.original_residual_receivable.alias('original_residual_receivable'),
	LKP_MASTER_OUT1.contract_oid_from_contract_equipment_summary.alias('contract_oid_from_contract_equipment_summary'),
	LKP_MASTER_OUT1.equipment_cost.alias('equipment_cost'),
	LKP_MASTER_OUT1.contract_oid_from_rpt_renewal_status.alias('contract_oid_from_rpt_renewal_status'),
	...
	LKP_MASTER_OUT1.customer_ssn_tax_id_number.alias('customer_ssn_tax_id_number'),
	LKP_MASTER_OUT1.customer_business_individual_indicator.alias('customer_business_individual_indicator'),
	...
	LKP_MASTER_OUT1.udfcontract_year_of_libor_used.alias('udfcontract_year_of_libor_used'),
	LKP_MASTER_OUT1.last_month_end_date.alias('last_month_end_date'),
	(LKP_MASTER_OUT1.deferred_income_lease .cast(numeric)).alias('deferred_income_lease'),
	(LKP_MASTER_OUT1.ltd_earned_income_lease .cast(numeric)).alias('ltd_earned_income_lease'),
	(LKP_MASTER_OUT1.ltd_earned_income_residual .cast(numeric)).alias('ltd_earned_income_residual'),
	...
	(when((((LKP_MASTER_OUT1.termination_date == None) |(Len(trim(LKP_MASTER_OUT1.termination_date)) == lit(0))) &(trim(LKP_MASTER_OUT1.is_booked) == lit('True')) ),(LKP_MASTER_OUT1.accumulated_fee .cast(numeric) )).otherwise(lit(0))).alias('fasb_fee_cur_bal'),
	...
	(when(((LKP_MASTER_OUT1.last_rental_paid_date == None) |(Len(trim(LKP_MASTER_OUT1.last_rental_paid_date)) == lit(0)) ),(None )).otherwise(LKP_MASTER_OUT1.last_rental_paid_date .cast(date))).alias('last_pmt_pst_dt'),
	(when(((LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt == None) |(Len(trim(LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt)) == lit(0)) , None ).otherwise(IFF((LKP_MASTER_OUT1.renewal_maturity_date_lti_rpt .cast(date) > LKP_MASTER_OUT1.maturity_date) )).otherwise((LKP_MASTER_OUT1.maturity_date )).otherwise(None))).alias('last_mat_extn_dt'),
	...
	(when((trim(LKP_MASTER_OUT1.product_code) == lit('LOAN') ),(LKP_MASTER_OUT1.ytd_interest_income )).otherwise(lit(0))).alias('ytd_bank_shr_int_paid_amt'),
	(when((trim(LKP_MASTER_OUT1.product_code) == lit('LOAN') ),(LKP_MASTER_OUT1.ytd_accrued_interest )).otherwise(lit(0))).alias('ytd_int_accr_amt'),
	LKP_MASTER_OUT1.current_payment_amount.alias('pmt_due_amt'),
	(when((trim(LKP_MASTER_OUT1.product_code) == lit('LOAN') , (when(((LKP_MASTER_OUT1.interest_mtd) IS NOT None ).otherwise((LKP_MASTER_OUT1.interest_mtd) ).otherwise(lit(0))) ).otherwise((when(((LKP_MASTER_OUT1.mtd_earned_income_residual) IS NOT None )).otherwise(((LKP_MASTER_OUT1.mtd_earned_income_residual) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.mtd_earned_income_lease) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.mtd_earned_income_lease) )).otherwise(lit(0)))))).alias('mtd_ln_int_paid_amt'),
	(when((LKP_MASTER_OUT1.OPEN_CLOSE_CD == lit('C') , lit(0) ).otherwise(when((LKP_MASTER_OUT1.BANK_SHR_BAL < lit(2) ).otherwise(lit(0) ).otherwise(when((Matdtnew1 <= AsOfDt )).otherwise((LKP_MASTER_OUT1.principal_balance ))).otherwise((IFF(PiDueAmt > lit(15) ))).otherwise((PiDueAmt )).otherwise(lit(0)))))).alias('pdue_pi_amt'),
	(when((LKP_MASTER_OUT1.OPEN_CLOSE_CD == lit('C') , None ).otherwise(when((LKP_MASTER_OUT1.BANK_SHR_BAL < lit(2) ).otherwise(None ).otherwise(when((PiDueAmt < lit(15) )).otherwise((None ))).otherwise((IFF(OldRentDueDt <= LKP_MASTER_OUT1.'{DATA_DATE}' ))).otherwise((OldRentDueDt )).otherwise(None))))).alias('pdue_pi_dt'),
	(RegUnfunded + LKP_MASTER_OUT1.CUR_BAL).alias('reg_exp_amt'),
	(when((((LKP_MASTER_OUT1.termination_date == None) |(Len(trim(LKP_MASTER_OUT1.termination_date)) == lit(0))) ),(TotPdueFeeAmt .cast(numeric) )).otherwise(lit(0))).alias('tot_pdue_fee_amt'),
	...
	(when(((LKP_MASTER_OUT1.BANK_SHR_BAL < lit(0)) &(RegBnkShrUnfd > lit(0)) , RegBnkShrUnfd ).otherwise(IFF((LKP_MASTER_OUT1.BANK_SHR_BAL > lit(0)) &(RegBnkShrUnfd < lit(0)) )).otherwise((LKP_MASTER_OUT1.BANK_SHR_BAL )).otherwise(LKP_MASTER_OUT1.BANK_SHR_BAL + RegBnkShrUnfd))).alias('reg_bank_shr_exp_amt'),
	LKP_MASTER_OUT1.current_contract_receivable.alias('current_contract_receivable'),
	...
	LKP_MASTER_OUT1.BANK_SHR_BAL.alias('BANK_SHR_BAL'),
	(when((trim(LKP_MASTER_OUT1.product_code) == lit('LOAN') , (when(((LKP_MASTER_OUT1.interest_ytd) IS NOT None ).otherwise((LKP_MASTER_OUT1.interest_ytd) ).otherwise(lit(0))) ).otherwise((when(((LKP_MASTER_OUT1.ytd_earned_income_residual) IS NOT None )).otherwise(((LKP_MASTER_OUT1.ytd_earned_income_residual) ))).otherwise((lit(0)) +(IFF((LKP_MASTER_OUT1.ytd_earned_income_lease) IS NOT None ))).otherwise(((LKP_MASTER_OUT1.ytd_earned_income_lease) )).otherwise(lit(0)))))).alias('ytd_int_paid_amt'),
	LKP_MASTER_OUT1.oldest_rent_due_date_contract_aging.alias('oldest_rent_due_date_contract_aging')
).filter("ACCT_ID <> 0")
...
spark.sql('drop table if exists ASPIRE_CREDIT_FCLTY_BAL_INTERMEDIATE')
SA_CUSTOMER_DS.write.saveAsTable(ASPIRE_CREDIT_FCLTY_BAL_INTERMEDIATE)
...
spark.sql('drop table if exists ASPIRE_CREDIT_FCLTY_BAL_INTERMEDIATE_test')
SA_CUSTOMER_DS.write.saveAsTable(ASPIRE_CREDIT_FCLTY_BAL_INTERMEDIATE_test)